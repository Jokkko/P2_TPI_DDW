...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function actualizarTotal() {
        ...
        // Hamburguesas
        document.querySelectorAll('.seccion-hamburguesas .cantidad-input').forEach(input => {
            const cantidad = parseInt(input.value);
            if (cantidad > 0) {
                const precio = parseInt(input.dataset.precio);
                const nombre = input.closest('.hamburguesa-card').querySelector('h3').textContent;
        ...
        // Acompañamientos
        document.querySelectorAll('.seccion-sides .cantidad-input').forEach(input => {
            const cantidad = parseInt(input.value);
            if (cantidad > 0) {
                const precio = parseInt(input.dataset.precio);
                const nombre = input.closest('.hamburguesa-card').querySelector('h3').textContent;
        ...
        // Bebidas
        document.querySelectorAll('.seccion-bebidas .cantidad-input').forEach(input => {
            const cantidad = parseInt(input.value);
            if (cantidad > 0) {
                const precio = parseInt(input.dataset.precio);
                const nombre = input.closest('.hamburguesa-card').querySelector('h3').textContent;
